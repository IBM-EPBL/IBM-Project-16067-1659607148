...
def view_sale():
    ...
    if request.method=="GET":
        ...
        query = '''select item_id,sum(quantity) from sale_items where sale_id in (select sale_id from sale where sale_date<= \'{}\' and sale_date>=\'{}\') group by(item_id)'''.format(end,start)
        exec_query = ibm_db.exec_immediate(conn, query)
        row = ibm_db.fetch_both(exec_query)
        while(row):
